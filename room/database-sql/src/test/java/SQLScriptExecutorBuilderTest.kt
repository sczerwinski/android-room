...
@DisplayName("Tests for SQLExecutor.Builder")
class SQLScriptExecutorBuilderTest {
    ...
    @Test
    @DisplayName(
        value = "GIVEN SQL script, " +
            "WHEN build SQLExecutor AND executeOn, " +
    ...
    @DisplayName(
        value = "GIVEN SQL script, " +
            "WHEN build SQLExecutor using function AND executeOn, " +
            "THEN execute all SQL statements on the database"
    )
    fun buildWithFunction() {
        val sqlScript = """
            insert into users(id, username, password) values (1, 'root', 'qwerty');
            insert into user_roles(user_id, role_id) values (1, 1);
            update roles set name='Admin' where id is 1;
        ...
        val sqlExecutor = SQLScriptExecutor {
            +sqlScript
        }
